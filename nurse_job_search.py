...
# Optional Google Sheets imports (for when we want to use them)
try:
    ...
    print("Google Sheets functionality not available (optional)")
...
def get_google_sheets_service():
    ...
    if not GOOGLE_SHEETS_AVAILABLE:
        logger.warning("Google Sheets functionality is not available. Skipping Google Sheets setup.")
        return None
...
def search_nurse_jobs_with_date_filter(date_filter_option="today", hours_old=12):
    """
    Search for new nurse jobs with flexible date filtering options.
    ...
    try:
        # Check if we're in GitHub Actions environment or local development
        is_github_actions = os.getenv('GITHUB_ACTIONS') == 'true'
        
        if is_github_actions or not GOOGLE_SHEETS_AVAILABLE:
            # GitHub Actions or local testing mode - skip Google Sheets
            existing_jobs = set()  # Empty set for CSV-only mode
            logger.info("CSV-ONLY MODE: Skipping Google Sheets duplicate checking (GitHub Actions or no credentials)")
        else:
            # Local development with Google Sheets available
            try:
                existing_jobs = get_existing_jobs()
                logger.info(f"Found {len(existing_jobs)} existing jobs in Google Sheet")
            except Exception as e:
                logger.warning(f"Could not access Google Sheets: {str(e)}. Using CSV-only mode.")
                existing_jobs = set()
        
        # Search for nurse jobs (both remote and non-remote)
        logger.info(f"Searching for nurse jobs from {filter_description}...")
        jobs = scrape_jobs(
            site_name=["indeed", "linkedin", "zip_recruiter", "glassdoor", "google"],
            search_term="nurse",
            location="United States",  # Nationwide search
            distance=50,  # 50 miles radius (default)
            results_wanted=500,  # Increased from 100 to 500 for more comprehensive results
        ...
        logger.info(f"Scraped {len(jobs_df)} total nurse jobs")
        ...
        # Filter out jobs with Indeed URLs in job_url_direct (keep only direct company links)
        # OPTIONS FOR FILTERING:
        # 1. Strict: Remove all Indeed URLs (current setting)
        # 2. Moderate: Keep jobs but prefer non-Indeed URLs
        # 3. None: Keep all jobs regardless of URL type
        
        FILTER_MODE = "strict"  # Change to "moderate" or "none" for more results
        
        if not jobs_df.empty and 'job_url_direct' in jobs_df.columns and FILTER_MODE == "strict":
            initial_count = len(jobs_df)
            # Keep jobs where job_url_direct does NOT contain 'indeed.com' OR is null/empty
            jobs_df = jobs_df[
                (jobs_df['job_url_direct'].isna()) | 
                (jobs_df['job_url_direct'] == '') | 
                (~jobs_df['job_url_direct'].str.contains('indeed.com', case=False, na=False))
            ]
            filtered_count = initial_count - len(jobs_df)
            logger.info(f"DIRECT URL FILTER (STRICT): Removed {filtered_count} jobs with Indeed URLs, kept {len(jobs_df)} jobs with direct company links")
        
        elif not jobs_df.empty and 'job_url_direct' in jobs_df.columns and FILTER_MODE == "moderate":
            # Add a flag to identify direct vs Indeed URLs but keep all jobs
            jobs_df['is_direct_company_url'] = (
                (jobs_df['job_url_direct'].notna()) & 
                (jobs_df['job_url_direct'] != '') & 
                (~jobs_df['job_url_direct'].str.contains('indeed.com', case=False, na=False))
            )
            direct_count = jobs_df['is_direct_company_url'].sum()
            logger.info(f"DIRECT URL FILTER (MODERATE): Found {direct_count} jobs with direct company links out of {len(jobs_df)} total jobs")
        
        elif FILTER_MODE == "none":
            logger.info(f"DIRECT URL FILTER (NONE): Keeping all {len(jobs_df)} jobs regardless of URL type")
        
        else:
            logger.warning("⚠️ 'job_url_direct' column not found or DataFrame is empty")
        ...
        # For CSV-only mode, we'll use all jobs (no duplicate filtering)
        new_jobs_df = jobs_df.copy()
        
        logger.info(f"Found {len(new_jobs_df)} nurse jobs from {filter_description} for CSV export")
        
        # Google Sheets update (if available and not in GitHub Actions)
        if not is_github_actions and GOOGLE_SHEETS_AVAILABLE and len(new_jobs_df) > 0:
            try:
                update_google_sheet(new_jobs_df)
                logger.info(f"✅ Successfully added {len(new_jobs_df)} nurse jobs to Google Sheets")
            except Exception as e:
                logger.warning(f"Could not update Google Sheets: {str(e)}. Continuing with CSV-only mode.")
        elif len(new_jobs_df) == 0:
            logger.info(f"No new nurse jobs from {filter_description} to add")
        else:
            logger.info(f"✅ Found {len(new_jobs_df)} nurse jobs - Google Sheets update skipped (GitHub Actions mode)")
        
        # Save CSV file (main output for testing)
        if not os.path.exists('job_results'):
            os.makedirs('job_results')
        
        filename = f"job_results/nurse_jobs_{date_filter_option}_{timestamp}.csv"
        new_jobs_df.to_csv(filename, quoting=csv.QUOTE_NONNUMERIC, escapechar="\\", index=False)
        logger.info(f"📄 CSV RESULTS SAVED: {filename}")
        logger.info(f"📊 SUMMARY: {len(new_jobs_df)} nurse jobs exported to CSV")
        
        # Show a preview of the results
        if len(new_jobs_df) > 0:
            logger.info(f"🔍 PREVIEW: First few jobs found:")
            for i, job in new_jobs_df.head(3).iterrows():
                logger.info(f"  - {job.get('title', 'N/A')} at {job.get('company', 'N/A')} ({job.get('location', 'N/A')})")
        
        return new_jobs_df
        
    except Exception as e:
        logger.error(f"Error in search_nurse_jobs_with_date_filter: {str(e)}")
        raise

def search_nurse_jobs():
    """Search for new nurse jobs posted today only."""
    return search_nurse_jobs_with_date_filter(date_filter_option="today")
...
if __name__ == "__main__":
    ...
    search_nurse_jobs_with_date_filter(date_filter_option="today")
    
    # Uncomment one of the following for different filtering options:
    
    # Option 2: Only jobs posted yesterday
    # search_nurse_jobs_with_date_filter(date_filter_option="yesterday")
    
    # Option 3: Jobs from last 12 hours (no additional date filtering)
    # search_nurse_jobs_with_date_filter(date_filter_option="last_12h")
    
    # Option 4: Jobs from last 24 hours (original behavior)
    # search_nurse_jobs_with_date_filter(date_filter_option="last_24h")
    
    # Option 5: Custom hours (e.g., last 6 hours)
    # search_nurse_jobs_with_date_filter(date_filter_option="custom_hours", hours_old=6) 